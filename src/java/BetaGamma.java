...
*/
//package edu.ksu.cis.kdd.util.stat.distribution;

//import cern.jet.math.Polynomial; // Roby Joehanes' patch -- no need to import now
/**
 ...
 * 
 * <P>Note this class is taken from cern.jet.stat.Gamma, but it's renamed to BetaGamma
 * to avoid naming conflict -- Roby Joehanes
 *
 * @author wolfgang.hoschek@cern.ch
 * @version 0.9, 22-Jun-99
 */
public class BetaGamma extends /*cern.jet.math.*/Constants {  // Roby Johanes' patch -- no need to supply fully qualified name
/**
...
static double powerSeries( double a, double b, double x ) throws ArithmeticException {
	...
	if( (a+b) < MAXGAM && Math.abs(u) < MAXLOG ) {
	        t = BetaGamma.gamma(a+b)/(BetaGamma.gamma(a)*BetaGamma.gamma(b));
	        s = s * t * Math.pow(x,a);
	    } else {
	       t = BetaGamma.logGamma(a+b) - BetaGamma.logGamma(a) - BetaGamma.logGamma(b) + u + Math.log(s);
	       if( t < MINLOG ) 	s = 0.0;
	       else  	            s = Math.exp(t);
	    }
	return s;
