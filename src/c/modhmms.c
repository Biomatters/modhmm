...
int main(int argc, char* argv[])
{
 ...
 user_defined_emission_score = NO;
  ...
  if(freqfile != NULL) {
    ...
    printf("Input error: A frequency file (-q, --freqfile=filename) must be specified with this scoring method\n");
    exit(0);
  }
  ...
  if(query == SEQ) {
	...
	if(save_hmm == YES) {
	  strcpy(hmmout_name, pure_seq_name_a);
	  strcat(hmmout_name, "_hmm_out.hmg");
	  strcat(hmmout_name, "\0");
	  if((hmmoutfile = fopen(hmmout_name, "w")) == NULL) {
	    perror(hmmout_name);
	    continue;
	  }
	  else {
	    savehmm(hmmoutfile, &retrain_hmm);
	    fclose(hmmoutfile);
	  }
	}
		...
		int print_log_odds, int print_reversi, int print_path, FILE *outfile, int run_globmem_scanner)
{
  ...
  if(print_log_like == YES) {
    ...
    /* print to outfile */
    fprintf(outfile, "Normalized log likelihood: %.4Lf\n", norm_log_like);
  ...
  if(print_post_probs == YES && is_tm_prot == YES) {
	...
	*(post_prob_label_matrix + get_mtx_index(i,label_nr,hmmp->nr_labels)) +=  
	...
	fprintf(outfile, "%.3Lf\t", *(post_prob_label_matrix + get_mtx_index(i,j,hmmp->nr_labels)));
      }
      fprintf(outfile, "\n");
    }
    fprintf(outfile, "\n");
